/**
 * Implementation for inlined string functions in 1.6.0
 */
#if BOTW_VERSION == 160
...
template <> void BufferedSafeStringBase<char>::assureTerminationImpl_() const {
    const_cast<BufferedSafeStringBase*>(this)
        ->getMutableStringTop_()[mBufferSize - 1] = this->cNullChar;
}
template <>
SafeStringBase<char>&
SafeStringBase<char>::operator=(const SafeStringBase<char>& other) = default;

template <>
SafeStringBase<char16>& SafeStringBase<char16>::operator=(
    const SafeStringBase<char16>& other) = default;
