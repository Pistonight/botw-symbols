...
bool File::open() {
    ...
    nn::Result result =
        nn::fs::OpenFile(&m_handle, m_path,
...
bool File::write(const FileBuffer& buffer) {
    ...
    result = nn::fs::WriteFile(
        m_handle, m_offset, buffer.content(), buffer.len(),
...
s64 File::read(FileBuffer& buffer) {
    ...
    nn::Result result = nn::fs::ReadFile(&readSize, m_handle, m_offset,
                                         buffer.last(), 1024 - buffer.len());
    if (result.IsFailure()) {
        return -1;
    }
